  ...
  return (
    <div className="app-shell">
      ...
      <div className="chat-panel">
        ...
        {messages.map((m, i) => {
          if (m.proposal) return <ProposalCard key={i} proposal={m.proposal} sessionId={sessionId} razorpayKeyId={razorpayKeyId} onSettled={handleSettled} />
          if (m.relax) return <RelaxationCard key={i} relax={m.relax} disabled={loading} onChoose={sendMessage} />
          if (m.fallback) return <FallbackCard key={i} fallback={m.fallback} />
          if (m.receipt) return <ReceiptCard key={i} receipt={m.receipt} />
          if (m.upsell) return <UpsellCard key={i} upsell={m.upsell} sessionId={sessionId} razorpayKeyId={razorpayKeyId} onDone={handleUpsellSettled} />
