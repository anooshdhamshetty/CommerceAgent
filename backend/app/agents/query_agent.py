...
- category: if the request plausibly matches one of the categories above, use that exact category value.
  ...
  guess a category from the list above just to force a match if it genuinely isn't a fit.
...
- attribute: ANY OTHER descriptive qualifier the user gave that is NOT the brand — scent, color, material,
  size, "wireless" vs "wired", etc. Capture it even if you're not sure the store carries that exact variant;
  it is the reasoning agent's job to check that later, not yours. Leave this null if the user gave no
  non-brand qualifier at all.
...
JSON fields exactly: category, attribute, brand, quantity, budget_cap"""
